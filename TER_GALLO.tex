...
\newline\boxXx{
	\begin{theoreme}[V. Milman, 1971]\label{thmMil}
		Pour toute $\varepsilon>0$, il existe une constante $c>0$ tel que pour tout $n\in\mathbb{N}$ et pour tous compact convexe symétrique $K\subset \mathbb{R}^n$, il existe un sous espace $V\subset  \mathbb{R}^n$ tel que :
		\begin{enumerate}
...
\boxXx{
\begin{definition}
	Soit $(X,||.||_X),(Y,||.||_Y)$ deux espaces normés et $C> 0$, on dis que $X$ s'injecte $C$-continûment dans $Y$, si il existe $T\in\mathcal{L}(X,Y)$ tel que pour tous $x\in X$
	\begin{equation*}
...
\newline\boxXx{
	\begin{theoreme}
	Pour tout $\varepsilon>0$ il existe $c>0$ tel que pour toute $n\in \mathbb{N}$ et pour toute norme $||.||$ sur $\mathbb{R}^n$ ,  $l^k_2$ s'injecte $(1+\varepsilon)$-continûment dans $(\mathbb{R}^n,||.||)$ pour un $k\geq c.\log(n)$. 
	\end{theoreme}
}
\begin{notation}
...
\begin{enumerate}
	...
	\end{equation*}
	Soit $r=\sup\big\{\lambda>0 \; ; \; \lambda(V\cap B^n_2)\subset K\cap V\big\}$, alors :
	\begin{equation*}
		\frac{1}{1+\varepsilon}T(B_2^k)\subset r(V\cap B^n_2) \subset K\cap V
	\end{equation*}
	d'où $T(B_2^k)\subset (1+\varepsilon)r(V\cap B^n_2) $ et finalement :
	\begin{equation*}
		r(V\cap B^n_2) \subset K\cap V \subset (1+\varepsilon)r(V\cap B^n_2) 
	\end{equation*}
\end{enumerate}
\newpage
...
\end{notation}
Montrons quelques propriétés qui serons utile par la suite.
\newline\boxXx{
	\begin{lemme}
...
\begin{proof}
	Soit $M \in O(n)$ et $x\in S^{n-1}$ alors la mesure définis par
	\begin{equation*}
...
\begin{notation}
	Si il n'y a pas d'ambiguïté sur la norme $||.||$ de $\mathbb{R}^n$ utilisé on noteras :
	\begin{enumerate}
...
\begin{proof}
	...
	\begin{array}{ccc}
		...
		\vdots&\vdots&\vdots\\
		\alpha = y_0 + \sum_{i=1}^{N-1}y_i\Big(\prod_{1\leq k\leq i}\lambda_k\Big)+\tilde{\alpha}\prod_{1\leq k\leq N}\lambda_k &\text{\hspace{4mm} avec \hspace{2mm}}&  \text{$\forall i\leq N$\hspace{1mm}} \lambda_i\leq \theta, y_i\in A \text{\hspace{2mm}et\hspace{2mm}} \tilde{\alpha}\in S^{n-1}\\
		\vdots & \vdots & \vdots
	\end{array}
	\end{equation*} 
	Si l'on pose $S_N = y_0 + \sum_{i=1}^{N}y_i\Big(\prod_{1\leq k\leq i}\lambda_k\Big)$, alors il existe $\tilde{\alpha}\in S^{n-1}$ tel que :
	\begin{equation*}
		|\alpha-S_N| \leq |\lambda_1 ... \lambda_N| |\tilde{\alpha}-y_N|\leq \theta^{N+1} \to 0  \text{\hspace{3mm}avec\hspace{2mm}}  N\to \infty
	\end{equation*}
...
\begin{proof} 
	...
	\end{array}
	\end{equation*}
	et pour tous $y\in V \backslash\{0\}$ on a 
	\begin{equation*}
		\begin{array}{cc}
			E\frac{1}{\sqrt{1+\varepsilon}}\leq\big|\big|\frac{y}{|y|}\big|\big|\leq E \sqrt{1+\varepsilon}\\
			E\frac{1}{\sqrt{1+\varepsilon}}|y|\leq||y||\leq E|y|\sqrt{1+\varepsilon}
 		\end{array}
	\end{equation*}
...
\begin{proof}
	...
	\begin{enumerate}
		...
		\item[-] $f(\theta)=2(3/\theta)^{c(\theta)(E/b)^2n}e^{-\eta^2 n /2}=2\exp\big(-\frac{\eta^2n}{4}\big)$
	\end{enumerate}
...
Considérons un compact convexe symétrique $K\subset \mathbb{R}^n$ et $T\in GL(n)$ tel que $S^{n-1}$ est l'ellipsoïde de volume maximale inscrite dans $TK$ alors si l'on montre le théorème 1.2 pour $TK$, c'est à dire que l'on montre qu'il existe $V$ avec $\dim V> c.\log(n) $ et 
\begin{equation*}
(V\cap B^n_2) \subset TK\cap V \subset (1+\varepsilon)(V\cap B^n_2) 
\end{equation*}
alors $W=T^{-1}V$ vérifie $\dim W = \dim V$ et 
\begin{equation*}
(W\cap T^{-1}(B^n_2)) \subset K\cap W \subset (1+\varepsilon)(W\cap T^{-1}(B^n_2)) 
\end{equation*}
Soit $r=\inf\big\{\lambda>0 \; ; \; \lambda(W\cap B^n_2)\subset K\cap W\big\}$, alors 
\begin{equation*}
r(W\cap B^n_2)\subset K\cap W \subset r(1+\varepsilon)(W\cap B^n_2)
\end{equation*}
Donc $K$ vérifie aussi le \ref{thmMil}.
