...
\newtheoremstyle{stm}{1mm}{1mm}{Ubuntu}{0pt}{Ubuntu}{-}{2mm}{} 
...
\titlespacing{\chapter}{5pt}{5pt plus 4pt minus 2pt}{5pt plus 2pt minus 2pt}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\boxXx{
	\begin{lemme}
		Soient $\alpha\in S^{n-1}$ , A un $\delta$-net pour un $1>\delta>0$, alors il existe $(y_i)_{i\in\mathbb{N}}\subset A$ et $(\beta_i)_{i\in\mathbb{N}}\subset \mathbb{R}^{+}$ tel que 
		\begin{equation*}
			\alpha = \sum_{i=0}^{+\infty}y_i \beta_i \text{\hspace{4mm} et \hspace{4mm} $\forall i\in \mathbb{N}$, }\; \beta_i\leq \delta^i
		\end{equation*}
	\end{lemme}
}
\begin{proof}
	Comme $A$ est un $\delta$-net alors il existe $y_0\in A$ tel que $|\alpha-y_0|<\delta$, et donc
	\begin{equation*}
		\alpha = y_0 + \lambda_1 \alpha'
	\end{equation*}
	avec $\lambda_1= |\alpha-y_0|\leq \delta$ et $\alpha' = \frac{\alpha-y_0}{\lambda_1}\in S^{n-1}$, on peut donc itéré le même procédé sur $\alpha'$ et réitéré indéfiniment : 
	\begin{equation*}
	\begin{array}{ccc}
		\alpha = y_0 + \lambda_1 (y_1+\lambda_2\alpha'')=y_0+\lambda_1y_1+\lambda_1\lambda_2\alpha'' &\text{\hspace{4mm} avec \hspace{2mm}}&  \lambda_2\leq \delta, \; y_1\in A \text{\hspace{2mm}et\hspace{2mm}} \alpha''\in S^{n-1}\\
		\vdots&\vdots&\vdots\\
		\alpha = y_0 + \sum_{i=1}^{N-1}y_i\Big(\prod_{1\leq k\leq i}\lambda_k\Big)+\tilde{\alpha}\prod_{1\leq k\leq N}\lambda_k &\text{\hspace{4mm} avec \hspace{2mm}}&  \text{$\forall i\leq N$\hspace{1mm}} \lambda_i\leq \delta, y_i\in A \text{\hspace{2mm}et\hspace{2mm}} \tilde{\alpha}\in S^{n-1}\\
		\vdots & \vdots & \vdots
	\end{array}
	\end{equation*} 
	Si l'on pose $S_N = y_0 + \sum_{i=1}^{N}y_i\Big(\prod_{1\leq k\leq i}\lambda_k\Big)$, alors il existe $\tilde{\alpha}\in S^{n-1}$ tel que :
	\begin{equation*}
		|\alpha-S_N| \leq |\lambda_1 ... \lambda_N| |\tilde{\alpha}-y_N|\leq \delta^{N+1} \to 0  \text{\hspace{3mm}avec\hspace{2mm}}  N\to \infty
	\end{equation*}
	il ne reste plus qu'as poser $\beta_0=1$ et pour $i>0$, $\beta_i=\prod_{1 \leq k\leq i}\lambda_k\leq \delta^i$ et l'on a :
	\begin{equation*}
...
\boxXx{
	\begin{lemme}
		$\forall \varepsilon >0$ , il existe $1>\theta>0$ tel que pour tous $n\in N$ , si l'on a $A$ un $\theta$-net sur $S^{n-1}$ et $||.||$ une norme sur $\mathbb{R}^n$, tel que:
		\begin{equation*}
			\forall  x \in A, \hspace{5mm} (1-\theta)E\leq \big|\big|x \big|\big|\leq (1+\theta)E
		\end{equation*}
		alors ,
		\begin{equation*}
			\forall  x \in \mathbb{R}^n, \hspace{5mm} (1-\varepsilon)|x|E\leq \big|\big|x\big|\big|\leq (1+\varepsilon)|x|E
		\end{equation*}
		de plus si $\varepsilon\leq \frac{1}{9}$, on peu prendre $\theta=\frac{\varepsilon}{9}$
	\end{lemme}
}
\begin{proof} 
	Soient $1>\theta>0$, $A$ un $\theta$-net sur $S^{n-1}$ et $x\in S^{n-1}$ par le lemme 4,  il existe $(y_i)_{i\in\mathbb{N}}\subset A$ et $(\beta_i)_{i\in\mathbb{N}}\subset \mathbb{R}^{+}$ tel que 
	\begin{equation*}
	x = \sum_{i=0}^{+\infty}y_i \beta_i \text{\hspace{4mm} et \hspace{4mm} $\forall i\in \mathbb{N}$, }\; \beta_i\leq \theta^i
	\end{equation*}
	
	\begin{align*}
		||x|| &= \big|\big| \sum_{i=0}^{+\infty}y_i \beta_i\big|\big|\\
		&\leq \sum_{i=0}^{+\infty}\theta^i ||y_i||\\
		& \leq  \sum_{i=0}^{+\infty}\theta^i (1+\theta)E=\frac{1+\theta}{1-\theta}E
	\end{align*}
	de même :
	\begin{align*}
		||x|| &\geq||y_0||- ||x-y_0||\\
		&\geq E(1-\theta)- \sum_{i=1}^{+\infty}\theta^i ||y_i||\\
		&\geq E\big((1-\theta)- \theta\frac{1+\theta}{1-\theta}\big)= E \frac{1-3\theta}{1-\theta}
	\end{align*}
	Il suffit donc de prendre $\theta$ tel que
	\begin{equation*}
	\begin{array}{cc}
		1+\varepsilon\geq \sqrt{1+\varepsilon}\geq \frac{1+\theta}{1-\theta}\\
		1-\varepsilon\leq\frac{1}{\sqrt{1+\varepsilon}}\leq \frac{1-3\theta}{1-\theta}
	\end{array}
	\end{equation*}
...
\begin{proof}
	Soit $\varepsilon>0$ , on se donne un $1>\theta>0$ donné par le lemme 5 et
	\begin{enumerate}
		\item[-] $V_0\subset \mathbb{R}^n$ un sous espace avec $\dim V_0 = k$
		\item[-]$A$ un $\theta$-net sur $V_0\cap S^{n-1}$, avec $|A|<\big(\frac{3}{\theta}\big)^{k}$ par le lemme *
		\item[-] $\eta = \frac{\theta E}{b}$
	\end{enumerate}
	où $k$ est choisi tel que $|A|< \frac{1}{2}e^{\frac{\eta^2n}{2}}< (\frac{3}{\theta})^k$.
	Dans un premier temps nous allons montrer qu'il existe $T\in O(n)$ tel que pour tous $x\in A$ 
	\begin{equation*}
		(1-\theta) E \leq ||Tx|| \leq (1+ \theta)
	\end{equation*}
	On a l'inégalité suivante : 
	\begin{align*}
	\nu\Big(\cap_{x\in A}\big\{T\in O(n)\; ; \;  \big| ||Tx||-E \big|\leq b\eta\big\}\Big)& = 1-\nu\big(\cup_{x\in A}\big\{T\in O(n)\; ; \; \big| ||Ty||-E \big|>b\eta\big\}\big)& \text{pour un $y\in A$}\\
	& \geq 1-|A|\nu\big(T\in O(n)\; ; \; \big| ||Ty||-E \big|>b\eta\big)\\
	...
	\end{equation*}
	Par le lemme 5 appliqué au $\theta$-net $TA$, pour tous $x\in \mathbb{R}^n$
	\begin{equation*}
	(1-\varepsilon)|x| E\leq ||x||\leq (1+\varepsilon)|x| E
	\end{equation*}
	...
	\begin{align*}
		k> \frac{1}{log(\frac{3}{\theta})}\Big(\frac{\theta^2 E^2}{2 b^2}n-log(2)\Big)
	\end{align*}
	
	\color{red}\textit{toujours pas réussi a conclure !} \color{black}
\end{proof}
