...
\newpage
\chapter{Partie 3}
\section{Estimation de la borne inférieur du maximum d'un vecteur gaussien}
\color{blue}
Dans la suite pour estimer $E$ nous aurons besoin d'une minoration de $\mathbb{E}\Big[\max_{1\leq i \leq N}g_i\Big]$ pour des $g_i$ i.i.d suivant $\mathcal{N}(0,1)$, nous démontrons une telle borne dans cette section.  
\newline\lemma{\textit(ratio de Mill) Soit $g$ une variable aléatoire suivant une loi $\mathcal{N}(0,1)$, alors \begin{equation*}
	\mathbb{P}(g>t) \geq \exp\big(-\frac{t^2}{2}\big)\frac{1}{\sqrt{2\pi}t+2}
	\end{equation*}}
\color{red}\begin{proof}
	
\end{proof}\color{black}\color{blue}
\theorem{ $\exists A,B>0$ tel que pour tout $N>1$, et $(g_1,...,g_N)$ des variables aléatoires i.i.d suivant une loi $\mathcal{N}(0,1)$, on est  :
	\begin{equation*}
	\mathbb{E}\Big[\max_{1\leq i \leq N}g_i\Big] \geq A\sqrt{\log N}- B
	\end{equation*}	
}
\begin{proof}
	...
	Pour $x\geq 2 $ on a $\exp(-K\sqrt{\log x })\leq \exp(-K\sqrt{\log 2}) $, en effet comme
	\begin{equation*}
	\big(\exp(-K\sqrt{\log x })\big)'=-\frac{K}{2\sqrt{\log x}}\exp(-K\sqrt{\log x } )<0
	\end{equation*} 
	par la suite posons $a =: 1 - \exp(-K\sqrt{\log 2})\approx 0,123$
	\begin{equation*}
	\mathbb{E}\Big[\max_{1\leq i \leq N}g_i\Big] \geq \big(\sqrt{2\log N-2 \log\log N}+\sqrt{\frac{2}{\pi}}\big)a-\sqrt{\frac{2}{\pi}}
	\end{equation*}
	\begin{equation*}
	\mathbb{E}\Big[\max_{1\leq i \leq N}g_i\Big] \geq a \sqrt{2\log N-2 \log\log N}- \sqrt{\frac{2}{\pi}}(1-a)
	\end{equation*}
	\begin{equation*}
	\mathbb{E}\Big[\max_{1\leq i \leq N}g_i\Big] \geq a C(N)\sqrt{\log N}- \sqrt{\frac{2}{\pi}}(1-a)
	\end{equation*}
	\begin{equation*}
	\mathbb{E}\Big[\max_{1\leq i \leq N}g_i\Big] \geq a C(e^e)\sqrt{\log N}- \sqrt{\frac{2}{\pi}}(1-a)
	\end{equation*}
	posons finalement $A = a C(e^e) \approx 0.138$ et $B = \sqrt{\frac{2}{\pi}}(1-a)\approx 0,699$ alors on a :
	\begin{equation*}
	\mathbb{E}\Big[\max_{1\leq i \leq N}g_i\Big] \geq A\sqrt{\log N}-B
	\end{equation*}
\end{proof}
\color{black}
\section{Estimation de E}
...
\begin{proof}
	...
	\begin{equation*}
		E\geq (2e\sqrt{n})^{-1} \mathbb{E}\big[\max_{1\leq i \leq [n/2]}|g_i|\big]\geq \frac{(2e\sqrt{n})^{-1}}{A\sqrt{\log(n/2)}-B}
	\end{equation*}
	\color{black}
\end{proof}
